    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 h-[calc(100vh-6rem)] overflow-y-auto pr-4 pb-10">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                <div>
                    <Button variant="ghost" onClick={() => navigate("/workers")} className="mb-2 pl-0 hover:bg-transparent hover:text-primary h-auto">
                        â† Volver
                    </Button>
                    <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-[#0d141b] dark:text-white">
                        {worker.firstName} {worker.lastName}
                    </h1>
                    <div className="flex flex-wrap gap-x-2 gap-y-1 text-sm text-muted-foreground mt-1">
                        <span>{worker.documentType}: {worker.documentNumber}</span>
                                        ...
                                        <TableRow key={assignment.id}>
                                            ...
                                            <TableCell>
                                                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${assignment.status === 'READ'
                                                    ...
                                                    {assignment.status === 'READ' ? 'Leído' : 'Asignado'}
                                                </span>
                                            </TableCell>
                                        </TableRow>
                                    ))}
