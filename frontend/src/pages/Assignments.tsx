...
import AssignmentsImportModal from "@/components/assignments/AssignmentsImportModal"
...
export default function Assignments() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm("Â¿Estás seguro de eliminar esta asignación? El dosímetro quedará DISPONIBLE.")) return;
        try {
    ...
    return (
        <>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-[#0d141b] dark:text-white">Asignaciones</h2>
                    <p className="text-sm md:text-base text-muted-foreground">Gestiona la entrega de dosímetros por periodo.</p>
                </div>
                <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center w-full md:w-auto">
                    ...
                    <div className="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                        ...
                        >
                            ...
                            <span className="hidden sm:inline">{generating ? "Generando..." : "Generar QR"}</span>
                        ...
                        <Button onClick={() => setIsFormOpen(true)} disabled={!selectedPeriodId} className="gap-2 flex-1 sm:flex-none">
                                    ...
                                    <TableBody>
                                        {assignments.map((assignment) => (
                                            <TableRow key={assignment.id}>
                                                ...
                                                <TableCell>{assignment.worker?.documentType}: {assignment.worker?.documentNumber}</TableCell>
                                                <TableCell className="text-sm text-muted-foreground">{assignment.useArea || '-'}</TableCell>
                                                <TableCell>
