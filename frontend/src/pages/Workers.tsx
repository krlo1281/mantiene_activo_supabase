...
export default function Workers() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm("Â¿Estás seguro de eliminar este usuario?")) return;
        try {
    ...
    return (
        <>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-[#0d141b] dark:text-white">Usuarios</h2>
                    <p className="text-sm md:text-base text-muted-foreground">Gestión de personal asignado a dosimetría.</p>
                </div>
                                    ...
                                    <TableBody>
                                        {workers.map((worker) => (
                                            <TableRow key={worker.id}>
                                                <TableCell className="font-medium">{worker.documentType}: {worker.documentNumber}</TableCell>
