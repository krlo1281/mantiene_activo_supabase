...
export default function AssignmentForm({ open, onOpenChange, onSuccess, selectedPeriod }: AssignmentFormProps) {
    ...
    const [useArea, setUseArea] = useState<string>("")

    const [branches, setBranches] = useState<Branch[]>([])
    const [existingAreas] = useState<string[]>([])
        ...
        if (open) {
            ...
            const loadData = async () => {
                try {
                    ...
                    // We removed useArea from worker, we need to get existing areas from assignments.
                    // But since we don't have assignments loaded here easily, we can just leave it as an open text input
                    // or fetch assignments and extract. Let's just keep the input without datalist for now,
                    // or fetch assignments to populate datalist.
                } catch (error) {
    ...
    const handleWorkerChange = (value: string) => {
        ...
        if (selectedWorker) {
            setUseArea(""); // No longer have default area from worker
            // Auto-select company if worker has only one
            if (selectedWorker.companies && selectedWorker.companies.length === 1) {
                handleCompanyChange(selectedWorker.companies[0].id);
            } else {
                handleCompanyChange(""); // Reset if multiple or none, force manual select
            }
        } else {
            setUseArea("");
            handleCompanyChange("");
    ...
    const handleCompanyChange = async (value: string) => {
        ...
        if (value) {
            try {
                const branchData = await getBranches(value);
                ...
                <form onSubmit={handleSubmit} className="grid gap-4 py-4">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Select onValueChange={handleWorkerChange} value={workerId}>
                            ...
                            <SelectContent>
                                {workers.map((w) => (
                                    <SelectItem key={w.id} value={w.id}>{w.firstName} {w.lastName} ({w.documentType}: {w.documentNumber})</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                    ...
                    {companyId && branches.length > 0 && (
                        <div className="grid gap-2">
                            <Label htmlFor="branch">Sede (Opcional)</Label>
                    ...
                    <div className="grid gap-2">
                        <Label htmlFor="useArea">Ãrea de Uso</Label>
                        <Input 
                            id="useArea" 
                            name="useArea" 
                            list="assignment-use-areas" 
                            value={useArea} 
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setUseArea(e.target.value)} 
                            placeholder="Ej: Rayos X" 
                        />
                        <datalist id="assignment-use-areas">
