...
export default function Layout() {
    ...
    return (
        <div className="flex flex-col md:flex-row min-h-screen w-full bg-background-light dark:bg-background-dark font-display text-[#0d141b] dark:text-white overflow-x-hidden">
            ...
            <aside className={`fixed md:sticky top-0 left-0 z-50 h-[100dvh] w-64 flex-col justify-between bg-white dark:bg-[#1a2632] border-r border-[#e7edf3] dark:border-[#2a3a4a] transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} flex`}>
                ...
                <div className="p-4 border-t border-[#e7edf3] dark:border-[#2a3a4a]">
                    <a className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#e7edf3] dark:hover:bg-[#2a3a4a] transition-colors cursor-pointer" onClick={handleLogout}>
            ...
            <main className="flex-1 flex flex-col min-w-0 bg-background-light dark:bg-background-dark overflow-y-auto">
                <div className="flex-1 p-4 md:p-6 lg:p-10 max-w-[1400px] mx-auto w-full">
                    <Outlet />
                </div>
            </main>
        </div>
